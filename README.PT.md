...
```bash
cd find_dups_pthon
python3 find_dups_python.py /caminho/para/scan1 /caminho/para/scan2 ...
```
...
| Tempo de hashing       | 1:55.751   | 2:07.520   | 1:31.992   |
| Tempo total            | 5:26.243   | 3:55.664   | 3:06.040   |
| Duplicatas encontradas | 0          | 696        | 696        |
| Workers/threads        | 12         | 12         | 12         |

**Notas**:
- Tempos no formato `minutos:segundos.milissegundos`
- A implementação Python encontrou 0 duplicatas enquanto Go e Rust encontraram 696, indicando um possível bug na lógica de detecção de duplicatas da versão Python
...
1. **Discrepância Python**: A versão Python encontrou 0 duplicatas enquanto Go/Rust encontraram 696. Isso precisa ser investigado — provavelmente relacionado às diferentes contagens de arquivos (149.044 vs 148.819) ou um bug na lógica de detecção de duplicatas.
...
- **Para script/prototipagem rápida**: Python — mais fácil de modificar, mas primeiro investigue o bug de detecção de duplicatas

## Melhorias Futuras

1. **Corrigir detecção de duplicatas Python** — investigar a discrepancia entre implementações
...
- Corrigir o problema de detecção de duplicatas Python
