...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 3. Собираем файлы, для которых будем вычислять хеш (размер не уникален)
    ...
    // 4. Вычисляем хеши с прогресс-баром
    ...
    // 5. Обновляем хеши в by_size (переносим из временного вектора)
    // Создаём map по пути для быстрого доступа
    ...
    // 6. Формируем CSV и bash-скрипт
    let mut csv_writer = csv::Writer::from_path("duplicates.csv")?;
    csv_writer.write_record(&["Path", "Size", "Hash", "CreationTime", "ModificationTime"])?;
